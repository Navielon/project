    ...
    <div>
        ...
        <div v-else> 
            <h2 id="reset_send" style="text-align: center; color: #3c3c3c;">
                Ожидайте письмо на <b class="mail">{{input.email}}</b> с дальнейшими инструкциями!
            </h2>
        </div>
    </div>
</template>
        ...
        methods: {
            emitClick() {
                this.$recaptcha('reset').then((token) => {
                    this.$root.$emit('send_websocket', `{"packet":"account.reset","data":{"recaptcha_v3":"${token}", "login":"${this.input.login}","mail":"${this.input.email}"}}`)
                });
            }, 
            listener(packet, data) {
                if(packet != 'account.reset') {
                    return;
                }
