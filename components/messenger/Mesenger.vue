<template>
    <div>
        <div v-bind:class="{ preloader_malc: isMessengerOpen }">
            <div id="loader-wrapper" v-if="!loaded">
                <div id="loader">
                    <p>LOADING</p>
            ...
            <div v-else> 
                <div class="messenger">
                    <h4 style="padding: 15px;color: #b1b0b0;margin-top: 0px;background: rgba(8,8,8,.09)">
                        ЛИЧНЫЕ СООБЩЕНИЯ
                        <div class="close-modal" v-on:click="handleClose">×</div>
                    </h4>


                    <div class="im_dialogs_col_wrap noselect" style="height: 100%;margin-top: -10px;">
                        <div class="im_dialogs_panel">
                            <div class="im_dialogs_search">
                                <input class="form-control im_dialogs_search_field no_outline ng-pristine ng-untouched ng-valid ng-empty" type="search" placeholder="Search" ng-model="search.query" autocomplete="off">
                                <a class="im_dialogs_search_clear tg_search_clear ng-hide" ng-click="searchClear()" ng-show="search.query.length">
                                    <i class="icon icon-search-clear"></i>
                                </a>
                            </div>
                        </div>

                        <div class="im_dialogs_col" style="height: 100%;">
                            <div class="im_dialogs_wrap nano has-scrollbar active-scrollbar">
                                <div class="im_dialogs_scrollable_wrap nano-content" ref="dialogsScroll" v-on:scroll.passive="handleScrollDialogs" style="right: -15px;">

                                    <ul class="nav nav-pills nav-stacked">
                                
                                        <li v-for="dialog in dialogs" v-bind:key="dialog.id" v-on:click="openDialog(dialog.id)" class="im_dialog_wrap">
                                            <a v-bind:class="{im_dialog: true, current_dialog: dialog.id==currentDialog}">
                                                <div class="im_dialog_meta pull-right text-right">
                                                    <div class="im_dialog_date">
                                                        {{getDate(dialog.last_changes)}}
                                                    </div>
                                                    <span class="im_dialog_badge badge">43</span>
                                                    <i class="im_dialog_unread ng-hide"></i>
                                                    <i class="im_dialog_pinned ng-hide"></i>
                                                </div>

                                                <i class="icon icon-select-tick"></i>

                                                <div class="im_dialog_photo pull-left peer_photo_init">
                                                    <img class="im_dialog_photo" v-bind:src="'http://api.loliland.ru/avatar/' + getDalogName(dialog.users, dialog.name)">
                                                </div>

                                                <div class="im_dialog_message_wrap">
                                                    <div class="im_dialog_peer">
                                                        <span>
                                                            {{getDalogName(dialog.users, dialog.name)}}
                                                        </span>
                                                    </div>

                                                    <span class="im_short_message_text">
                                                        {{dialog.text}} fdgsdfgddf sdf fds sdf dfsdfs dsffsd dfs
                                                    </span>
                               ...
                               <div class="nano-pane" style="display: block;">
                                   <div class="nano-slider" v-bind:style="sliderStyle"></div>
                                </div>
                            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            handleClose () {
                this.$store.commit('setMessengerOpen', false);
                setTimeout(
					() => {
                        document.documentElement.style.overflow = 'auto';
                        this.$store.commit('setStopScroll', false);
					}, 750
				);
            },
            openDialog (dialogId) {
            ...
            handleScrollDialogs (e) {
                var currentScroll = this.$refs.dialogsScroll.scrollTop;
                var maxHeight = this.$refs.dialogsScroll.scrollHeight - this.$refs.dialogsScroll.clientHeight;
            ...
            updateScrollDialogs() {
                var currentScroll = this.$refs.dialogsScroll.scrollTop;
                var entryHeight = this.$refs.dialogsScroll.scrollHeight;
                var height = this.$refs.dialogsScroll.clientHeight;
                console.log(height);

                var start = entryHeight - height;
                if(start > 0) {
                    var length = height * height / (entryHeight);
                    ...
                    var scrollY = currentScroll * (height - length) / start;

                    this.sliderStyle = "height: "+length+"px; transform: translate(5px, "+scrollY+"px)";
                }
            },
            closeModalWithEsc (e) {
                if (e.keyCode === 27 && this.isMessengerOpen) {
            ...
            getDalogName(users, name) {
                return users[0];
            },
            getDate(value) {
                let d = new Date(value);
                let c = new Date();
                if(c.getDay() == d.getDay() && c.getMonth() == d.getMonth()) {
                    let format = "%H:%M";
                    return main.getDate(format, value);
                } else if(c.getDay() == d.getDay()+1 && c.getMonth() == d.getMonth()) {
                    let format = "Вчера, %H:%M";
                    return main.getDate(format, value);
                }
                let format = "%d %m %y";
                return main.getDate(format, value);
            ...
            listener(packet, data) {
                if(packet === 'messenger.getDialogs') {
                    for (let d in data) {
                        this.dialogs.push(data[d]);
                    }
...
<style scoped>
    .preloader_malc {
        ...
        right: 0;
        bottom: 0;
        background: rgba(1, 0, 15, 0.55) url(/assets/img/bg.png) no-repeat center center / cover;
        z-index: 99;
        overflow: hidden;
    }
    .messenger {
...
/*! * Generated using the Bootstrap Customizer (http://getbootstrap.com/customize/?id=ef8b2e1e74a6ba7423f1) * Config saved to config.json and https://gist.github.com/ef8b2e1e74a6ba7423f1 */
 ...
 *,:after,:before{
 ...
 :not(.no-enter)#toast-container>div.ng-enter,:not(.no-leave)#toast-container>div.ng-leave{
    -webkit-transition:200ms cubic-bezier(.25,.25,.75,.75) all;
    -moz-transition:200ms cubic-bezier(.25,.25,.75,.75) all;
    -ms-transition:200ms cubic-bezier(.25,.25,.75,.75) all;
    -o-transition:200ms cubic-bezier(.25,.25,.75,.75) all;
    transition:200ms cubic-bezier(.25,.25,.75,.75) all
}
 ...
 .btn-link.btn-link-noarea:hover,.btn-link.dropdown-toggle:hover{
    background:0 0
}
 ...
 .btn-md,.btn-md:focus,.btn-md:hover{
    color:#72a0c7
}
 .btn-md:hover{
    background:#f2f6fa
 ...
 .btn-md-danger,.btn-md-danger:focus,.btn-md-danger:hover{
    color:#c05f5a
}
 .btn-md-danger:hover{
    background:#fbe7e6
 ...
 .md-input-label span.ng-enter-prepare{
    display:none
}
 .md-input-animated .md-input-group,.md-input-animated .md-input-label{
 ...
 .md-input-group-centered,.md-input-group-centered .md-input{
    text-align:center
}
 ...
 a.md_modal_action:active,a.md_modal_action:focus,a.md_modal_action:hover{
    text-decoration:none
}
 .md_modal_title{
    font-weight:700;
 ...
 .navbar_offline .navbar-header,.navbar_offline .navbar-offline>li{
    float:left
}
 .navbar-offline{
    max-width:400px;
 ...
 .progress-arc-wrap{
    ...
    border-radius:100%;
    transform-origin:center center;
    -webkit-animation:infinite_rotation .8s linear infinite;
    -moz-animation:infinite_rotation .8s linear infinite;
    -ms-animation:infinite_rotation .8s linear infinite;
    animation:infinite_rotation .8s linear infinite
}
 ...
 .composer_progress_icon .progress-arc .progress-arc-bar{
    stroke:#b3b3b3
}
 .progress-arc-percent .progress-arc .progress-arc-bar{
    stroke:#FFF;
 ...
 .composer_progress_icon .stop100{
    stop-color:#b3b3b3
}
 ...
 .modal-backdrop.fade,.modal.fade,.modal.fade .modal-dialog{
    -webkit-transition:none;
    -moz-transition:none;
    -o-transition:none;
    transition:none
 ...
 .modal-close-link,.modal-head-link{
    font-size:12px;
    line-height:1.4;
    float:right;
    padding:0 2px;
 ...
 .modal-close-link:hover,.modal-head-link:hover{
    text-decoration:none
}
 .modal-head-link{
    margin-right:15px
 ...
 .modal_section_body dl,.modal_section_body p{
    margin:5px 0
}
 ...
 a.tg_checkbox:hover,p.tg_checkbox:hover{
    text-decoration:none
}
 ...
 .login_head_submit_progress,.login_head_submit_wrap{
    float:right
}
 .login_head_submit_wrap{
    font-weight:700
}
 .login_head_submit_btn,.login_head_submit_progress{
    font-size:13px;
    line-height:20px;
 ...
 .login_head_submit_btn:active,.login_head_submit_btn:focus,.login_head_submit_btn:hover{
    color:#fff;
    text-decoration:none
}
 .icon-next-submit{
 ...
 .logo_footer_learn_more_link,.logo_footer_learn_more_link:active,.logo_footer_learn_more_link:focus,.logo_footer_learn_more_link:hover{
    color:#84a2bc;
    font-weight:700
}
 .login_form_wrap{
 ...
 .login_footer_about_wrap{
    ...
    margin-top:12px;
    padding:44px 65px;
    -webkit-box-shadow:0 1px 0 #dfe5ec;
    -moz-box-shadow:0 1px 0 #dfe5ec;
 ...
 .login_footer_about_wrap h3 .login_footer_about_hide{
    color:#999;
    font-size:80%;
    font-weight:400;
    margin-left:8px
 ...
 .im_dialogs_scrollable_wrap a.im_dialog .glyphicon,.im_dialogs_scrollable_wrap a.im_dialog .icon{
    display:none
}
 .im_dialogs_scrollable_wrap .active a.im_dialog{
    border-radius:0;
 ...
 a.im_dialog .im_dialog_draft_from{
    color:#c05f5a
}
 ...
 .im_dialogs_scrollable_wrap a.im_dialog_searchpeer,.im_dialogs_scrollable_wrap a.im_dialog_searchpeer:hover,.im_dialogs_scrollable_wrap a.im_dialog_searchpeer_selected{
    background:#f3f3f3
}
 .im_dialog_peer{
    color:#dedede;
 ...
 .is_2x .im_dialog_pinned{
    background-size:40px 948px
}
 .active .im_dialog_pinned,a.im_dialog:hover .im_dialog_pinned,a.im_dialog_selected .im_dialog_pinned{
    background-position:-13px -916px
}
 .im_dialog_date{
    color:#b3b3b3;
 ...
 .im_dialog_message,.im_dialog_peer{
    overflow:hidden;
    white-space:nowrap;
    text-overflow:ellipsis
}
 ...
 .im_history_pinned_hide .icon-reply-bar{
    display:block;
    background:#999;
    width:18px;
    height:2px;
 ...
 .im_message_author_via,.im_message_fwd_via{
    color:#3a6d99
}
 .im_message_author_via:hover,.im_message_fwd_via:hover{
    text-decoration:none
}
 .im_message_author_via:hover .im_message_fwd_author,.im_message_fwd_via:hover .im_message_fwd_author{
    text-decoration:underline
}
 .non_osx .im_message_author_wrap,.non_osx .im_message_fwd_author_wrap{
    font-size:12px
}
 ...
 .im_message_contact_photo .peer_initials,.im_message_from_photo .peer_initials,.im_message_fwd_photo .peer_initials{
    line-height:42px
}
 a.im_message_contact_photo,a.im_message_from_photo{
    margin:0 15px 0 0
}
 ...
 .im_message_contact_name,.im_message_contact_phone{
    white-space:nowrap
}
 a.im_message_photo_thumb,a.im_message_video_thumb{
    display:block;
    overflow:hidden;
    border-radius:2px
}
 img.im_message_photo_thumb,img.im_message_video_thumb{
    overflow:hidden;
    border-radius:2px
}
 .im_message_video,a.im_message_photo_thumb{
    margin-top:5px
}
 ...
 .im_message_media_embed iframe,.im_message_media_embed webview{
    position:absolute;
    top:0;
    left:0;
    width:100%;
 ...
 .im_message_audio,.im_message_document,.im_message_upload_file,.im_message_video{
    margin-top:3px;
    width:317px
}
 .im_message_audio{
 ...
 .im_message_file_button_dl_audio .audio_player_btn_icon_pause,.is_1x .im_message_file_button_dl_audio .audio_player_btn_icon_pause{
    width:12px;
    height:14px;
    background-position:-14px -587px;
    margin:14px 15px
}
 .im_history_select_active .im_message_outer_wrap:hover .icon-document,.im_message_selected .icon-document{
    background-color:#dae6f0;
    background-position:-2px -542px
}
 .im_history_select_active .clickable,.im_history_select_active a,.im_history_select_active button{
    pointer-events:none
}
 ...
 .im_message_document_name:after{
    content:attr(data-name);
    display:block;
    overflow:hidden;
    text-overflow:ellipsis
}
 .im_message_document_name:before{
    content:attr(data-ext);
    float:right
}
 .im_message_document_name:hover:before{
    text-decoration:underline
}
 ...
 .audio_player_actions a,.im_message_document_actions a{
    margin-right:10px
}
 ...
 .audio_player_duration,.audio_player_size{
    color:#999;
    padding-left:2px
}
 ...
 .audio_player_media canvas{
    display:none
}
 .im_message_download_progress_wrap,.im_message_upload_progress_wrap{
    margin-top:5px;
    width:200px
}
 .im_message_document_thumbed .im_message_document_actions,.im_message_document_thumbed .im_message_document_name_wrap,.im_message_document_thumbed .im_message_download_progress_wrap,.im_message_document_thumbed .im_message_upload_progress_wrap{
    width:207px
}
 .im_message_document_thumbed .im_message_document_name{
    max-width:110px
}
 .im_message_video .im_message_document_actions,.im_message_video .im_message_document_name_wrap,.im_message_video .im_message_download_progress_wrap{
    width:152px
}
 .im_message_video .im_message_document_name_wrap{
    margin-top:5px
}
 .im_message_cancelable_progress_wrap,.im_message_playback_progress_wrap{
    margin-top:4px
}
 ...
 .tg_down_progress,.tg_up_progress{
    height:4px;
    margin:0;
    padding:0;
    background:rgba(218,228,234,.5);
 ...
 .im_message_date_text:hover{
    text-decoration:underline
}
 div.im_message_author,div.im_message_body{
    display:block;
    overflow:hidden
}
 .im_message_body{
 ...
 .im_history_select_active .im_message_outer_wrap:hover .icon-message-views,.im_message_selected .icon-message-views,.message_focus .icon-message-views{
 ...
 .im_message_reply_body .im_short_message_media,.im_message_reply_body .im_short_message_service{
    color:#999
}
 ...
 .im_message_reply_thumbed .im_message_reply_border,.im_message_reply_thumbed .im_message_reply_wrap{
    height:42px
}
 .im_message_reply_thumbed .im_message_reply_author{
    margin-top:4px
}
 .im_message_reply_thumbed .im_message_reply_author,.im_message_reply_thumbed .im_message_reply_body{
    margin-left:52px
}
 ...
 .im_message_document_caption,.im_message_photo_caption,.im_message_video_caption{
    margin-top:3px;
    clear:both;
    word-wrap:break-word;
    line-height:150%;
 ...
 .im_message_keyboard .reply_markup_wrap{
    margin-top:7px
}
 .reply_markup_row{
    padding:4px 0;
 ...
 .im_history_loaded .im_history_selected_wrap{
    display:block
}
 .im_page_peer_not_selected .im_history_selected_wrap{
    display:none
}
 .im_history_selected_loading_wrap{
    display:block
}
 .im_history_loaded .im_history_selected_loading_wrap,.im_page_peer_not_selected .im_history_selected_loading_wrap{
    display:none
}
 .im_history_empty,.im_history_not_selected{
    visibility:hidden;
    text-align:center;
    color:#999;
    font-size:16px;
 ...
 .im_history_empty.vertical-aligned,.im_history_not_selected.vertical-aligned{
    visibility:visible
}
 .im_history_to_bottom .im_history_empty,.im_history_to_bottom .im_history_not_selected{
    display:none
}
 .im_history_loading{
    width:35px;
 ...
 .im_history_loading.vertical-aligned{
    visibility:visible
}
 ...
 .im_history_loading_more .progress-arc-wrap{
    display:none
}
 .im_history_loading_more.im_history_loading_more_active{
    visibility:visible
}
 .im_history_loading_more.im_history_loading_more_active .progress-arc-wrap{
    display:block
}
 .im_history_loading_less{
    display:block;
 ...
 .im_history_loading_less .progress-arc-wrap{
    display:none
}
 .im_history_loading_less.im_history_loading_less_active{
    visibility:visible
}
 .im_history_loading_less.im_history_loading_less_active .progress-arc-wrap{
    display:block
}
 .im_send_panel_wrap{
    margin:0 auto;
 ...
 .im_attach_input,.im_media_attach_input{
    cursor:pointer;
 ...
 .im_send_reply_form_wrap a.im_panel_own_photo,.im_send_reply_form_wrap a.im_panel_peer_photo{
    margin-top:41px
}
 ...
 .img_fullsize_wrap,a.img_fullsize{
    display:block;
    text-align:center
}
 img.img_fullsize{
 ...
 .composer_progress_enabled-add,.composer_progress_enabled-remove{
 ...
 .composer_progress_enabled-add .composer_progress_icon_wrap,.composer_progress_enabled-remove .composer_progress_icon_wrap{
    transition:opacity cubic-bezier(.25,.46,.45,.94) .2s;
    display:block
}
 .composer_progress_enabled .composer_progress_icon_wrap{
    display:block;
    opacity:1
}
 .composer_emoji_insert_btn{
 ...
 .composer_progress_enabled .composer_emoji_insert_btn{
    display:none;
    opacity:0
}
 .composer_progress_enabled-add .composer_emoji_insert_btn,.composer_progress_enabled-remove .composer_emoji_insert_btn{
    transition:opacity cubic-bezier(.25,.46,.45,.94) .2s;
    display:block
}
 .icon-emoji{
 ...
 .composer_emoji_tooltip_tab:active,.composer_emoji_tooltip_tab:focus,.composer_emoji_tooltip_tab:hover{
    text-decoration:none
}
 .composer_emoji_tooltip_tab_emoji,.composer_emoji_tooltip_tabs_stickers_active .composer_emoji_tooltip_tab_stickers{
    color:#53a9ea
}
 .composer_emoji_tooltip_tab_stickers,.composer_emoji_tooltip_tabs_stickers_active .composer_emoji_tooltip_tab_emoji{
    color:#999
}
 .composer_emoji_tooltip_tab_shadow{
    ...
    height:1px;
    background:#53a9ea;
    -webkit-box-shadow:0 1px 0 0 #53a9ea;
    -moz-box-shadow:0 1px 0 0 #53a9ea;
    box-shadow:0 1px 0 0 #53a9ea;
    -webkit-transform:translate3d(0,0,0);
 ...
 .composer_emoji_tooltip_category.active,.composer_emoji_tooltip_category.active:hover{
    opacity:1
}
 ...
 .non_msie .composer_emoji_tooltip_tabs_contents{
    transition:transform ease-in-out .2s;
    -webkit-transform:translate3d(0,0,0);
    -moz-transform:translate3d(0,0,0);
    -o-transform:translate3d(0,0,0);
 ...
 .msie .composer_emoji_tooltip_tabs_contents{
    margin-left:0;
 ...
 .composer_dropdown_wrap{
    ...
    z-index:10;
    border:0;
    -webkit-box-shadow:0 1px 1px 0 rgba(60,75,87,.27);
    -moz-box-shadow:0 1px 1px 0 rgba(60,75,87,.27);
    box-shadow:0 1px 1px 0 rgba(60,75,87,.27);
    border-radius:0;
    margin-top:-5px;
    margin-left:-1px;
    width:380px
 ...
 .composer_dropdown li a:hover,.composer_dropdown li.composer_autocomplete_option_active a{
    color:#52719a;
    background:#f2f6fa;
    text-decoration:none
}
 ...
 .composer_dropdown li a:hover .composer_user_mention,.composer_dropdown li.composer_autocomplete_option_active a .composer_user_mention{
    color:#698192
}
 ...
 .composer_user_mention,.composer_user_name{
    display:inline-block;
    line-height:32px;
    vertical-align:top
}
 ...
 .composer_autocomplete_option_active a.composer_command_option .composer_command_desc,a.composer_command_option:hover .composer_command_desc{
    color:#698192
}
 ...
 .composer_rich_textarea:empty:active::before,.composer_rich_textarea:empty:focus::before{
    opacity:.75
}
 ...
 .composer_rich_textarea:empty:active:before,_:-ms-lang(x){
    display:none
}
 .composer_rich_textarea:empty:focus:before,_:-ms-lang(x){
    display:none
}
 .composer_command_btn{
    display:block;
 ...
 .inline_result_gif,.inline_result_photo,.inline_result_sticker{
    display:inline-block;
    padding:1px
}
 ...
 li a:hover .inline_article_thumb_initials,li.composer_autocomplete_option_active a .inline_article_thumb_initials{
    color:#698192;
    background:#e0e9f3
}
 ...
 .inline_article_description,.inline_article_thumb_wrap,.inline_article_title,.inline_article_url{
    pointer-events:none
}
 ...
 li a:hover .inline_article_description,li.composer_autocomplete_option_active a .inline_article_description{
    color:#698192
}
 .inline_article_url{
    padding-top:4px
}
 .composer_dropdown>li.inline_result_gif>a,.composer_dropdown>li.inline_result_photo>a,.composer_dropdown>li.inline_result_sticker>a{
    padding:0;
    line-height:0;
    display:block;
    overflow:hidden
}
 li.inline_result_gif.composer_autocomplete_option_active a,li.inline_result_photo.composer_autocomplete_option_active a,li.inline_result_sticker.composer_autocomplete_option_active a{
    position:relative
}
 li.inline_result_gif.composer_autocomplete_option_active a .inline_result_ind,li.inline_result_photo.composer_autocomplete_option_active a .inline_result_ind,li.inline_result_sticker.composer_autocomplete_option_active a .inline_result_ind{
    display:block;
 ...
 .inline_result_gif .img_gif_video,.inline_result_photo .inline_result_photo_image{
    object-fit:cover
}
 ...
 .inline_result_gif_http,.inline_result_gif_mtproto,.inline_result_photo_http,.inline_result_photo_mtproto{
    pointer-events:none
}
 .im_voice_recorder_wrap{
    display:none;
 ...
 .im_send_form_hover .im_recorder_label{
    color:#CCC
}
 .im_recorder_label_hover,.im_send_form_hover .im_recorder_label_hout{
    display:none
}
 .im_send_form_hover .im_recorder_label_hover{
    display:inline
}
 .im_processing_recording,.im_voice_recording{
    color:#AAA
}
 .im_processing_recording .im_voice_recorder_wrap,.im_voice_recording .im_voice_recorder_wrap{
    display:block
}
 ...
 .md_simple_modal_alert_body .error_modal_description,.md_simple_modal_alert_body .md_simple_header{
    text-align:center
}
 ...
 .error_modal_details_link,.error_modal_details_link:hover{
    font-size:11px;
    color:#999
}
 ...
 .document_modal_window,.photo_modal_window,.video_modal_window{
    display:block
}
 .photo_modal_error{
    color:#999;
 ...
 .contacts_modal_members_list .active a.contacts_modal_contact,.contacts_modal_members_list .active a.contacts_modal_contact:hover,.contacts_modal_members_list a.contacts_modal_contact:hover{
    background:#f2f6fa
}
 ...
 .contacts_modal_members_list .active a.contacts_modal_contact .contacts_modal_contact_status,.contacts_modal_members_list .active a.contacts_modal_contact .md_modal_list_peer_description,a.contacts_modal_contact:hover .contacts_modal_contact_status,a.contacts_modal_contact:hover .md_modal_list_peer_description{
    color:#91a6ba
}
 ...
 .confirm_modal_window .modal-dialog,.error_modal_window .modal-dialog,.md_simple_modal_window .modal-dialog{
    max-width:348px
}
 ...
 .user_color_8,.user_color_8:hover{
    color:#c07844
}
 .user_bgcolor_1{
    background:#cc90e2
 ...
 a.peer_photo_init:active,a.peer_photo_init:focus,a.peer_photo_init:hover{
    text-decoration:none
}
 .peer_initials{
    display:block;
 ...
 .im_dialogs_contacts_wrap h5,.im_dialogs_messages_wrap h5{
    color:#999;
    font-size:13px;
    margin-left:20px;
    font-weight:700
 ...
 .img_gif_meta_contents.ng-enter,.img_gif_meta_contents.ng-leave.ng-leave-active{
    opacity:0
}
 .img_gif_meta_contents.ng-enter.ng-enter-active,.img_gif_meta_contents.ng-leave{
    opacity:1
}
 ...
 .icon-cancel .icon-bar:first-child{
    -webkit-transform:rotate(-45deg);
    -moz-transform:rotate(-45deg);
    -o-transform:rotate(-45deg);
    -ms-transform:rotate(-45deg);
 ...
 .icon-cancel .icon-bar:last-child{
    -webkit-transform:translate3d(0,-2px,0) rotate(45deg);
    -moz-transform:translate3d(0,-2px,0) rotate(45deg);
    -o-transform:translate3d(0,-2px,0) rotate(45deg);
    -ms-transform:translate3d(0,-2px,0) rotate(45deg);
 ...
 .img_round_meta_contents.ng-enter,.img_round_meta_contents.ng-leave.ng-leave-active{
    opacity:0
}
 .img_round_meta_contents.ng-enter.ng-enter-active,.img_round_meta_contents.ng-leave{
    opacity:1
}
 ...
 .countries_modal_search_field:active,.countries_modal_search_field:focus{
    background-color:#fff
}
 ...
 a.peer_modal_photo,div.peer_modal_photo{
    display:block;
    margin-right:19px
}
 ...
 .md_modal_split_action:active .md_modal_split_action_camera,.md_modal_split_action:hover .md_modal_split_action_camera{
 ...
 .md_modal_split_action:active .md_modal_split_action_twitter,.md_modal_split_action:hover .md_modal_split_action_twitter{
 ...
 a.md_modal_list_peer_photo,div.md_modal_list_peer_photo{
    margin-right:12px
}
 ...
 .md_modal_list_peer_description .status_online,.md_modal_list_peer_description.status_online{
    color:#3a6d99
}
 .md_modal_footer{
    padding:15px 27px
 ...
 @media print{
    body,html{
     ...
     body .im_bottom_panel_wrap,body .im_dialogs_col_wrap,body .tg_head_logo_wrap,body .tg_head_peer_dropdown,body .tg_head_peer_return_btn,body .tg_head_peer_search_btn{
        display:none
    }
     ...
     body .im_history_col_wrap,body .tg_head_main_wrap{
        width:auto;
        max-width:700px;
        float:none;
        margin:0 auto
